...
function Dashboard(props) {
	...
	const URL_EVENTS_LIST = getUrl("EVENTS_LIST");
	const URL_EVENT = getUrl("EVENT");

    useEffect(
        () => {
            window.initFlowbite();
            if (dashboard.stats === null) {
                props.loadEventsDashboard();
            }
        }
    )
    ...
    // +++ Components - events by month 
    const { byMonth } = dashboard.events;

    let eventsByType = [];
    if (byMonth !== null) {
        for (var i = byMonth.length - 1; i >= 0; i--) {
            let eventData = []
            const { type, stats, data } = byMonth[i];
        ...
        for (var i = all.length - 1; i >= 0; i--) {
            ...
            eventsAll.push(
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <Link to={generatePath(URL_EVENT, {id:item.id})}><th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {item.title}
                    </th></Link>
                    <td class="px-6 py-4">
                        {item.client_display}
		...
		<div className="px-4 py-4">
			<div class="relative overflow-x-auto border border-gray-300 shadow-md bg-white p-4 rounded-lg">
			    <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
			        ...
			        <Link to={URL_EVENTS_LIST} class="inline-flex items-center text-gray-500 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-3 py-1.5 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
			        	All Events
			        </Link>
...
const mapStateToProps = state => ({
    dashboard: state.event.dashboard,
})

export default connect(mapStateToProps, { loadEventsDashboard })(Dashboard);
